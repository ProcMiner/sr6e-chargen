...
export function FinishingStepsStep({ rules, data, onChange }: Props) {
  ...
  function spendKarmaOnRating(id: string, field: "connection" | "loyalty") {
    ...
    if (!contact || (!primeOrElite && contact[field] >= charisma) || contactKarmaBudget < 1) return;
  ...
  return (
    <div className="lifepath-builder">
      ...
      <p className="hint">
        ...
        {primeOrElite ? "further, with no Charisma cap for Elite (house rule)" : `up to your Charisma (${charisma})`}.
                ...
                <div className="contact-rating-editor">
                  {(["connection", "loyalty"] as const).map((field) => (
                    <label key={field} className="inline-field">
                      ...
                      />
                      {(primeOrElite || c[field] < charisma) && contactKarmaBudget >= 1 && (
